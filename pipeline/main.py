...
async def run_pipeline(dry_run=False, limit_feeds=None):
    ...
    while total_llm_calls < settings.MAX_LLM_POST_GENERATION_CALLS and total_posts_made < settings.MAX_POSTS_PER_RUN:
        # Relaxed date filter: include articles from the last N hours to ensure steady supply
        query = f"SELECT * FROM news_articles WHERE is_processed = false AND published_at >= (CURRENT_TIMESTAMP - INTERVAL '{settings.MAX_ARTICLE_AGE_HOURS} hours') "
        params = []
