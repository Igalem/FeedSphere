...
You are taking part in a LIVE PUBLIC DEBATE on FeedSphere against ${opponentName}.
The topic is based on this news article: "${article.title}"

Your job is to deliver a punchy, opinionated argument IN CHARACTER. Be bold, be biased, be entertaining.

Rules:
- Stay completely in character at all times.
- Max 2 sentences. Be direct and punchy.
- Never start with "I think" or "In my opinion".
- Never use '—', '--', or '-'. Use commas, colons, or periods instead.
- DO NOT add hashtags in the argument text.
- YOUR FINAL OUTPUT MUST BE A VALID JSON OBJECT.

JSON Structure:
{
  "argument": "Your punchy debate argument (max 2 sentences)",
  "sentiment_score": 0-100
}`;

  const questionPrompt = `Generate a short, catchy, engaging debate question (max 10 words) based on this article title: "${article.title}"
The question should be provocative, shareable, and framed so two opposing sides can argue it.
Examples: "Is $100 Luffy Pillow a Scam or Collector's Gold?", "Will AI Replace Human Writers?", "Is Bitcoin Dead or Just Getting Started?"
Return ONLY the question text, nothing else.`;

  const userMsg = [{ role: 'user', content: `Article: "${article.title}"\nSummary: ${article.snippet || ''}\n\nDeliver your debate argument now. JSON only.` }];

  const [responseA, responseB, questionRaw] = await Promise.all([
    generateLLMResponse(buildPrompt(agentA, agentB.name), userMsg, { maxTokens: 300, temperature: 0.9 }),
    generateLLMResponse(buildPrompt(agentB, agentA.name), userMsg, { maxTokens: 300, temperature: 0.9 }),
    generateLLMResponse(questionPrompt, [{ role: 'user', content: 'Generate the debate question now.' }], { maxTokens: 60, temperature: 0.8 }),
  ]);
